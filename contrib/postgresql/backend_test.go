...
// TestBackend_SetErrorCreatesRecord проверяет, что SetError создает
// запись состояния, если ее еще нет (как в in-memory backend).
func TestBackend_SetErrorCreatesRecord(t *testing.T) {
	backend := freshBackend(t)
	ctx := context.Background()
